...
def get_matchups():
    """Fetch eligible upcoming football matchups for the current scan window."""
    response = session.get(
        f"{BASE_URL}/0.1/sports/29/matchups",
    ...
    matchups = payload.get("matchups", []) if isinstance(payload, dict) else payload
    now_ts = time.time()
    result = {}

    for matchup in matchups:
        matchup_id = matchup.get("id")
        start_time = matchup.get("startTime") or matchup.get("start_time")
        league = matchup.get("league") or {}
        league_name = (
            league.get("name", "") if isinstance(league, dict) else str(league)
        ) or matchup.get("leagueName", "")

        if not matchup_id or not start_time or is_blocked_league(league_name):
            continue
        ...
        hours_until_match = (start_ts - now_ts) / 3600
        if not 0 <= hours_until_match <= SCAN_HOURS:
            continue
        ...
        match_name = matchup.get("name") or " vs ".join(names)
        if not match_name or is_blocked_match(match_name):
            continue
