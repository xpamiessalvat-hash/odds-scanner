    ...
    for key, ep in episodes.items():
        ...
        if inactive_minutes >= EPISODE_RESET_MINUTES:
            ...
            if total_move >= MIN_EPISODE_MOVEMENT:
                ...
                # IMPORTANT: the target market is always the -1 side.
                # We compare its shortening against the +1 side, but we
                # never flip the signal to +1. A high dominance means the
                # steam is actually supporting the AH-1 side we want to test.
                ...
                # Observational FVS only; NOT a calibrated probability.
                ...
                timing_score = timing_score_fn(hours)
                fvs = clamp(
                    0.30 * selected_base
                    + 0.25 * dominance
                    + 0.20 * movement_score
...
def write_ah1_episode(
    ...
    print(
        f"📌 AH-1 #{ep['episode_id']} | {ep['league']} | {ep['match']} | "
