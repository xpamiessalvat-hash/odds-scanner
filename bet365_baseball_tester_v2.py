...
def section(body,home,away):
    """
    Troba NOMÉS una secció que contingui els dos equips del partit.
    Evita agafar quotes d'un altre partit.
    """
    ...
    for i,x in enumerate(lines):
        ...
        # Condició obligatòria: els DOS equips han de ser presents.
        if h not in sec_norm or a not in sec_norm:
            continue

        score=4
        if any(k in sec.lower() for k in [
            "baseball","run line","handicap","más de","menos de",
            "moneyline","línea de dinero"
        ]):
            score += 2

        positions=[sec_norm.find(h),sec_norm.find(a)]
