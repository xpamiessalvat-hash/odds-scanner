...
CSV_FILE = "football_episodes_fvs.csv"
...
def ensure_csv():
    ...
    with open(CSV_FILE, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            "league", "match", "matchup_id", "market", "side", "points",
            "start_odd", "end_odd", "movement_pct", "duration_minutes",
            "hours_until_match_start", "hours_until_match_end", "observations",
            "base_prob_pct", "steam_dominance_pct", "movement_score",
...
def process_market(
    ...
    # Build the current two-sided market snapshot so the FVS can use
    # a market-implied base probability instead of a fixed 55%.
    current_odds = {}
    current_points = {}
    for price_data in prices:
        side0 = price_data.get("designation")
        american0 = price_data.get("price")
        points0 = price_data.get("points")
        if american0 is None:
            continue
        if market_type == "moneyline":
            if side0 not in ("home", "draw", "away"):
                continue
            points0 = ""
        elif market_type == "spread":
            if points0 not in VALID_SPREADS:
                continue
        else:
            if points0 not in VALID_TOTALS:
                continue
        odd0 = american_to_decimal(american0)
        if odd0 is not None:
            current_odds[side0] = odd0
            current_points[side0] = points0

    inv = {side0: 1.0 / odd0 for side0, odd0 in current_odds.items() if odd0 > 1.0}
    inv_total = sum(inv.values())
    base_probs = {side0: (v / inv_total) * 100.0 for side0, v in inv.items()} if inv_total else {}

    movement_now = {}
    for side0, odd0 in current_odds.items():
        key0 = (matchup_id, market_type, side0, current_points[side0])
        old0 = previous_odds.get(key0)
        if old0 is not None and odd0 < old0:
            movement_now[side0] = ((old0 - odd0) / old0) * 100.0
        else:
            movement_now[side0] = 0.0
        ...
        try:
            ...
            if key not in episodes:
                episodes[key] = {
                    ...
                    "observations": 1,
                    "base_prob_pct": base_probs.get(side, 50.0),
                    "steam_dominance_pct": 50.0,
                }

                own_move = movement_now.get(side, 0.0)
                other_moves = [v for k2, v in movement_now.items() if k2 != side]
                other_move = max(other_moves) if other_moves else 0.0
                total_move = own_move + other_move
                episodes[key]["steam_dominance_pct"] = (
                    (own_move / total_move) * 100.0
            ...
            if decimal_odd < ep["last_odd"]:
                ...
                ep["observations"] += 1
                ep["base_prob_pct"] = base_probs.get(side, ep.get("base_prob_pct", 50.0))
                own_move = movement_now.get(side, 0.0)
                other_moves = [v for k2, v in movement_now.items() if k2 != side]
                other_move = max(other_moves) if other_moves else 0.0
                total_move = own_move + other_move
                if total_move > 0:
                    ep["steam_dominance_pct"] = (own_move / total_move) * 100.0
