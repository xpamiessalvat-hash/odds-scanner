# V10 AUTONOMOUS MODE
# - Quiet terminal: no recurring scan dumps.
# - Persistent daily log under logs/.
# - Meaningful MOVE/STEAM/VALUE alerts remain visible.
import requests
import time
...
_LOG_FILE = _LOG_DIR / f"steam_{datetime.now().strftime('%Y-%m-%d')}.log"
...
_sys.stdout = _Tee(_original_stdout, open(_LOG_FILE, "a", encoding="utf-8"))

print(f"📝 Log persistent: {_LOG_FILE}", flush=True)
...
while True:
    ...
    try:
        ...
        # V10: no imprimir recompte de partits a cada cicle.
